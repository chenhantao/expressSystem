	...
	<tbody>
		<tr>
			...
			<c:if test="${list.recipientProvince == province && list.recipientCity == city && list.recipientArea == area && list.orderState !=1}">
				<td><a href="${path }/courier/confirmreceipt?orderId=${list.orderId}"><button type="button" class="btn btn-success btn-xs">确认收货</button></a></td>
			</c:if>
        </tr>
	</tbody>
    </c:forEach>
        ...
        <ul class="pager">
    		...
    		<c:choose>
    			...
    			<c:otherwise>
    				<li><a href="${path}/courier/checkorder?page=${page.nextPage}&limit=${limit}">下一页&raquo;</a></li>
